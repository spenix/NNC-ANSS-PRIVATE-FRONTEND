...
// for Redux imports
import { addUserManualData, getUserManualData, editUserManualData } from "../../../redux/userManual/userManualActions";
...
const Details = (props) => {
    ...
    const [userManualId, setUserManualId] = useState();
        ...
        setTimeout(() => {
            if(isBase64(id) == false || checkFormAction() != true){
                ...
                if(formAction != "add-data"){
                    setUserManualId(window.atob(id));
                }
            }
        }, 1000);
        // dispatch(getAllData());
    ...
    useEffect(() => {
        if(userManualId){
            dispatch(getUserManualData({}, userManualId))
            .then(() =>{
    ...
    const getDataDetails = () => {
        if(Object.keys(userManual_details).length){
            const {attributes, id} = userManual_details;
    ...
    const onFinish = (values) => {
        ...
        if(formAction == "add-data")
            if(pageRoles.includes(ACTION_ADD)){
                dispatch(addUserManualData(formData)).then(() => {
                    setLoadAddUserBtn(false);
                    openNotificationWithIcon('success', "Success", "User Manual is added successfully.");
                    onCancelAction();
                }).catch((e) => {
                    errMsg();
                });
        ...
        if(formAction == "edit-data")
            if(pageRoles.includes(ACTION_EDIT)){
                dispatch(editUserManualData(formData, userManualId))
                .then(() =>{
                    setLoadAddUserBtn(false);
                    openNotificationWithIcon('success', "Success", "User Manual is updated successfully.");
                    setIsDataFetched(true);
                }).catch((e) => {
                    errMsg();
                });
    ...
    >
       ...
       <Row justify="space-between" gutter={[16, 16]} style={{padding:"10px"}}>
         <Col span={24} md={24}>
         <Card title={pageAction+" User Manual"} bordered={false}  style={cardStyle}>
