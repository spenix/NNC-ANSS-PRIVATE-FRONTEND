...
import { getAllData, getAllForTblData, deleteUserTemplateData } from "../../../redux/userTemplate/userTemplateActions";
...
export default function TableList(props) {
  ...
  const btnForAddData = () => {
    if(pageRoles.includes(ACTION_ADD)){
      ...
      return (
        ...
        <Link to={`/templates/forms/add-data/${window.btoa(0)}`}>
        <Button
            block
  ...
  const btnForViewData = (dataIndex) => {
    if(pageRoles.includes(ACTION_VIEW)){
      return (
        <Link
                to={`/templates/forms/view-data/${window.btoa(dataIndex)}`}
          >
  ...
  const btnForEditData = (dataIndex) => {
    if(pageRoles.includes(ACTION_EDIT)){
      return (
        <Link
                to={`/templates/forms/edit-data/${window.btoa(dataIndex)}`}
          >
  ...
  const processData = () => {
    const results = [];
    console.log(userTemplate_data);
    if(typeof userTemplate_data !== 'undefined'){
      for (let i = 0; i < userTemplate_data.length; i++) {
        results.push({
          key: userTemplate_data[i].id,
          document_url: userTemplate_data[i]?.attributes?.document_url,
          description: userTemplate_data[i]?.attributes.description,
          date_uploaded: moment(userTemplate_data[i]?.attributes.created_at).tz('Asia/Manila').format('LLL z'),
          actions: availableActions()
        });
      }
      setData(results);
      // if(Object.keys(UserTemplate_meta).length){
      //   setPagination({
      //     ...listParams.pagination,
      //     total: UserTemplate_meta.pagination.total, // total regardless of pagination
      //   });
      // }
    }
  ...
  const confirmDel = (id) => {
    if(pageRoles.includes(ACTION_DELETE)){
      dispatch(deleteUserTemplateData({}, id))
      .then(()=> {
        openNotificationWithIcon("success", "Success", "User Template was deleted successfully.")
        fetch();
      });
    }else{
  ...
  const handleButtonClick = (e, record) => {
    ...
    switch (data[0]) {
      case "View":
        // history.push(`/templates/forms/view-data/${window.btoa(record.key)}`);
        viewDataDetails(record);
        break;
      case "Edit":
        history.push(`/templates/forms/edit-data/${window.btoa(record.key)}`)
        break;
      case "Delete":
        showConfirm(record, "Are you sure you want to delete this indicator data template?");
        break;
      default:
        break;
    }
  ...
  const viewDataDetails = (detail) => {
   ...
   Modal.info({
     ...
     title: (
       <h5 className="hp-mb-0 hp-font-weight-500">View Indicator Data Template</h5>
     ),
     width:800,
     content: (
       ...
       <li><b>Description: </b>{detail?.description}</li>
           {
             detail?.document_url ? (
               <>
                 ...
                 {/* <li><b>Language: </b>{detail?.document_url}</li> */}
               </>
