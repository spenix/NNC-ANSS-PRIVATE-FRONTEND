...
// Redux
import { getAllData, getAllForTblData, deletePolicyStatusData } from "../../../redux/policy-status/policyStatusActions";
...
export default function UsersList(props) {
  ...
  const processData = () => {
    const results = [];
    if(typeof policyStatus_data !== 'undefined'){
      for (let i = 0; i < policyStatus_data.length; i++) {
        results.push({
          key: policyStatus_data[i].id,
          policyStatus: policyStatus_data[i]?.attributes?.display_name,
          status: policyStatus_data[i]?.attributes?.is_active,
          actions: availableActions()
        });
      }
      setData(results);
      if(Object.keys(policyStatus_meta).length){
        setPagination({
          ...listParams.pagination,
          total: policyStatus_meta.pagination.total, // total regardless of pagination
        });
      }
    }
  ...
  const confirmDel = (id) => {
        if(pageRoles.includes(ACTION_DELETE)){
          dispatch(deletePolicyStatusData({}, id))
          .then(()=> {
            openNotificationWithIcon("success", "Success", `Policy Status ${DELETED_DATA}`)
            fetch(pagination);
          }).catch((e) => {
            errMsg();
          });
  ...
  const btnForAddData = () => {
    if(pageRoles.includes(ACTION_ADD)){
      return (
        ...
        <Link to={`/policy-statuses/forms/add-data/${window.btoa(0)}`}>
                  <Button
                    block
  ...
  const btnForEditData = (dataIndex) => {
    if(pageRoles.includes(ACTION_EDIT)){
      return (
        <Link
                   to={`/policy-statuses/forms/edit-data/${window.btoa(dataIndex)}`}
              >
  ...
  const btnForDeleteData = (dataIndex) => {
    if(pageRoles.includes(ACTION_DELETE)){
      return (
        <Popconfirm
                placement="topLeft"
                title="Are you sure you want to delete this policy status?"
  ...
  const btnForViewData = (dataIndex) => {
    if(pageRoles.includes(ACTION_VIEW)){
      return (
        <Link
                   to={`/policy-statuses/forms/view-data/${window.btoa(dataIndex)}`}
              >
  ...
  const columns = [
    {
      ...
      sorter: (a, b) => a.policyStatus.localeCompare(b.policyStatus),
  ...
  const handleButtonClick = (e, record) => {
    ...
    switch (data[0]) {
      case "View":
        // history.push(`/policy-statuses/forms/view-data/${window.btoa(record.key)}`);
        viewDataDetails(record);
        break;
      case "Edit":
        history.push(`/policy-statuses/forms/edit-data/${window.btoa(record.key)}`)
        break;
      case "Delete":
        showConfirm(record, "Are you sure you want to delete this policy status?");
        break;
      default:
        break;
    }
  ...
  const viewDataDetails = (detail) => {
   Modal.info({
     ...
     title: (
       <h5 className="hp-mb-0 hp-font-weight-500">View Policy Status</h5>
     ),
     width:800,
     content: (
       <ul style={{ listStyle:"none" }}>
       <li><b>Status Name: </b>{detail?.policyStatus}</li>
    ...
    <>
      ...
      >
        <Col className="gutter-row" span={12}>
          <BreadCrumbs
              homePage = "Home"
              breadCrumbActive="Policy Status"
            />
