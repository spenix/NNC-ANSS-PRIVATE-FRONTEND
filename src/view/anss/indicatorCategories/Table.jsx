...
// Redux
import { getAllData, getAllForTblData, deleteIndicatorCategoryData } from "../../../redux/indicatorCategories/indicatorCategoriesActions";
...
export default function UsersList(props) {
  ...
  const processData = () => {
    const results = [];
    if(typeof indicatorCategory_data !== 'undefined'){
      for (let i = 0; i < indicatorCategory_data.length; i++) {
        results.push({
          key: indicatorCategory_data[i].id,
          name: indicatorCategory_data[i]?.attributes?.name,
          description: indicatorCategory_data[i]?.attributes?.description,
          is_active: indicatorCategory_data[i]?.attributes?.is_active,
          actions: availableActions()
        });
      }
      setData(results);
      if(Object.keys(indicatorCategory_meta).length){
        setPagination({
          ...listParams.pagination,
          total: indicatorCategory_meta.pagination.total, // total regardless of pagination
        });
      }
    }
  ...
  const confirmDel = (id) => {
    if(pageRoles.includes(ACTION_DELETE)){
      dispatch(deleteIndicatorCategoryData({}, id))
      .then(()=> {
        openNotificationWithIcon("success", "Success", `Indicator Category ${DELETED_DATA}`)
        fetch(pagination);
      }).catch((e) => {
        errMsg();
      });
  ...
  const btnForAddData = () => {
    if(pageRoles.includes(ACTION_ADD)){
      return (
        ...
        <Link to={`/indicator-categories/forms/add-data/${window.btoa(0)}`}>
                  <Button
                    block
  ...
  const btnForEditData = (dataIndex) => {
    if(pageRoles.includes(ACTION_EDIT)){
      return (
        <Link
              to={`/indicator-categories/forms/edit-data/${window.btoa(dataIndex)}`}
        >
  ...
  const btnForViewData = (dataIndex) => {
    if(pageRoles.includes(ACTION_VIEW)){
      return (
        <Link
                   to={`/indicator-categories/forms/view-data/${window.btoa(dataIndex)}`}
              >
  ...
  const viewDataDetails = (detail) => {
   ...
   Modal.info({
     ...
     title: (
       <h5 className="hp-mb-0 hp-font-weight-500">View Indicator Category</h5>
     ),
     width:800,
     content: (
  ...
  const handleButtonClick = (e, record) => {
    ...
    switch (data[0]) {
      case "View":
        // history.push(`/indicator-categories/forms/view-data/${window.btoa(record.key)}`);
        viewDataDetails(record);
        break;
      case "Edit":
        history.push(`/indicator-categories/forms/edit-data/${window.btoa(record.key)}`)
        break;
      case "Delete":
        showConfirm(record, "Are you sure to delete this indicator category?");
        break;
      default:
        break;
    }
