...
import {  PolicyProgrammesDataApproval } from "../../../redux/policy-programmes/policyProgrammesActions";
...
const AmsModal = (props) => {
  ...
  const {status, message} = useSelector((state) => state.policyProgrammes);
...
const clearErrMessage = () => {
  ...
  const onFinish = (values) => {
    confirm({
      title: `Are you sure you want to ${actionSelected} ${dataSelected.length > 1 ? "these" : "this"} policy & ${dataSelected.length > 1 ? "programmes" : "programme"}?`,
      icon: (
  ...
  const submitData = (values) => {
    ...
    for (let index = 0; index < dataSelected.length; index++) {
      ...
      dispatch(
        PolicyProgrammesDataApproval(params, dataSelected[index]?.id)
        ).then(() => {
  ...
  const modalBtn = () => {
    ...
    return ( 
      <Modal
      title={`POLICY & PROGRAMME TO BE ${actionSelected?.toUpperCase() == "APPROVE" ? "APPROVED" : "RETURNED"}`}
      width={800}
      visible={open}
        ...
        <Row>
          ...
          renderItem={(item, index) => (
            <List.Item key={index} style={{border: '1px solid #DCDCDC', padding: '10px', borderRadius: '10px', margin: '5px', boxShadow:'10px'}}>
              <List.Item.Meta
                // title={'Policy: '+ item?.attributes?.name}
                description={(
                  <ul style={{listStyle:"none", marginLeft: 0, paddingLeft: 0, color:"black"}}>
                    <li><b>Policy Classifcation: </b>{item?.attributes?.classification?.section}</li>
                    <li><b>Policy Indicator: </b>{item?.attributes?.environment?.indicator  }</li>
                    <li><b>Policy Name: </b>{item?.attributes?.name}</li>
                    <li><b>Policy Details: </b>{item?.attributes?.details}</li>
                    <li><b>Policy Status: </b>{item?.attributes?.status?.display_name}</li>
                    <li><b>Service Scale: </b>{(item?.attributes?.service_scale).toLowerCase() == "both" ? "Both, National & Sub-National" : item?.attributes?.service_scale == "Sub-national" ? "Sub-National" : item?.attributes?.service_scale}</li>
                    <li><b>Coverage (%): </b>{item?.attributes?.coverage}</li>
