...
export default function UsersList(props) {
  ...
  const processData = () => {
    const results = [];
    if(typeof memberState_data !== 'undefined'){
      for (let i = 0; i < memberState_data.length; i++) {
        results.push({
          key: memberState_data[i].id,
          name: memberState_data[i]?.attributes?.name,
          unsd: memberState_data[i]?.attributes?.unsd,
          iso_alpha: memberState_data[i]?.attributes?.iso_alpha,
          status: memberState_data[i]?.attributes?.is_active,
          actions: availableActions()
        });
      }
      setData(results);
      if(Object.keys(memberState_meta).length){
        setPagination({
          ...listParams.pagination,
          total: memberState_meta.pagination.total, // total regardless of pagination
        });
      }
    }
  ...
  const confirmDel = (id) => {
    if(pageRoles.includes(ACTION_DELETE)){
      dispatch(deleteMemberStateData({}, id))
      .then(()=> {
        openNotificationWithIcon("success", "Success", "Member State data is deleted successfully.")
        fetch(pagination);
      }).catch((e) => {
        errMsg();
      });
    ...
    if(pageRoles.includes(ACTION_ADD)){
      return (
        <Col className="gutter-row" span={7}>
          <Link to={`/countries/forms/add-data/${window.btoa(0)}`}>
                  <Button
                    block
  ...
  const btnForEditData = (dataIndex) => {
    if(pageRoles.includes(ACTION_EDIT)){
      return (
        <Link
                to={`/countries/forms/edit-data/${window.btoa(dataIndex)}`}
          >
  ...
  const btnForDeleteData = (dataIndex) => {
    if(pageRoles.includes(ACTION_DELETE)){
      return (
        <Popconfirm
                placement="topLeft"
                title="Are you sure to delete this member state?"
  ...
  const btnForViewData = (dataIndex) => {
    if(pageRoles.includes(ACTION_VIEW)){
      return (
        <Link
                to={`/countries/forms/view-data/${window.btoa(dataIndex)}`}
          >
  ...
  const columns = [
   
    {
      ...
      align: "left",
      sorter: true
      // ...getColumnSearchProps('iso_alpha'),
      // sorter: (a, b) => a.iso_alpha.length - b.iso_alpha.length,
  ...
  const handleButtonClick = (e, record) => {
    ...
    switch (data[0]) {
      case "View":
        ...
        break;
      case "Edit":
        history.push(`/countries/forms/edit-data/${window.btoa(record.key)}`)
        break;
      case "Delete":
        showConfirm(record, "Are you sure to delete this member state?");
        break;
      default:
        break;
    }
  ...
  const viewDataDetails = (detail) => {
    ...
    Modal.info({
      ...
      title: (
        <h5 className="hp-mb-0 hp-font-weight-500">View Member State</h5>
      ),
      width:800,
      content: (
        <ul style={{ listStyle:"none" }}>
           <li><b>ISO Alpha: </b>{detail?.iso_alpha}</li>
           <li><b>UNSD: </b>{detail?.unsd}</li>
          <li><b>Member State: </b>{detail?.name}</li>
