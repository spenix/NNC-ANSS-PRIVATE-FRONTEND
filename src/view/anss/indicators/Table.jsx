...
// Redux
import { getAllData, getAllForTblData, deleteIndicatorData } from "../../../redux/indicators/indicatorsActions2";
...
export default function UsersList(props) {
  ...
  const processData = () => {
    const results = [];
    if(typeof indicator_data !== 'undefined'){
      for (let i = 0; i < indicator_data.length; i++) {
        results.push({
          key: indicator_data[i].id,
          name: indicator_data[i]?.attributes?.name,
          description: indicator_data[i]?.attributes?.description,
          status: indicator_data[i]?.attributes?.is_active,
          indicator_type: indicator_data[i]?.attributes?.type?.name,
          data: indicator_data[i]?.attributes,
          actions: availableActions()
        });
      }
      setData(results);
      if(Object.keys(indicator_meta).length){
        setPagination({
          ...listParams.pagination,
          total: indicator_meta.pagination.total, // total regardless of pagination
        });
      }
    }
  ...
  const confirmDel = (id) => {
        if(pageRoles.includes(ACTION_DELETE)){
          dispatch(deleteIndicatorData({}, id))
          .then(()=> {
            openNotificationWithIcon("success", "Success",`Indicator Repository ${DELETED_DATA}`)
            fetch(pagination);
          }).catch((e) => {
            errMsg();
          });
    ...
    if(pageRoles.includes(ACTION_ADD)){
      return (
        <Col className="gutter-row" span={7}>
                <Link to={`/indicators/forms/add-data/${window.btoa(0)}`}>
                  <Button
                    block
  ...
  const btnForEditData = (dataIndex) => {
    if(pageRoles.includes(ACTION_EDIT)){
      return (
        <Link
                   to={`/indicators/forms/edit-data/${window.btoa(dataIndex)}`}
              >
  ...
  const btnForDeleteData = (dataIndex) => {
    if(pageRoles.includes(ACTION_DELETE)){
      return (
        <Popconfirm
                placement="topLeft"
                title="Are you sure to delete this indicator repository?"
  ...
  const btnForViewData = (dataIndex) => {
    if(pageRoles.includes(ACTION_VIEW)){
      return (
        <Link
                   to={`/indicators/forms/view-data/${window.btoa(dataIndex)}`}
              >
  ...
  const columns = [
    ...
    },
    {
      ...
      width: "15%",
      sorter: true,
      // sorter: (a, b) => a.indicator_type.localeCompare(b.indicator_type),
      //  sortDirections: ['descend', 'ascend'],
    },
    {
      title: "Status",
  ...
  const handleButtonClick = (e, record) => {
    ...
    switch (data[0]) {
      case "View":
        // history.push(`/indicators/forms/view-data/${window.btoa(record.key)}`);
        viewDataDetails(record);
        break;
      case "Edit":
        history.push(`/indicators/forms/edit-data/${window.btoa(record.key)}`)
        break;
      case "Delete":
        showConfirm(record, "Are you sure to delete this indicator repository?");
        break;
      default:
        break;
    }
  ...
  const viewDataDetails = (detail) => {
   ...
   Modal.info({
     ...
     title: (
       <h5 className="hp-mb-0 hp-font-weight-500">View Indicator Repository</h5>
     ),
     width:800,
     content: (
       <ul style={{ listStyle:"none" }}>
       <li><b>Code: </b>{detail?.data?.code}</li>
       <li><b>Indicator Name: </b>{detail?.name}</li>
       <li><b>Description: </b>{detail?.description}</li>
       <li><b>Indicator Type: </b>{detail?.indicator_type}</li>
       <li><b>Indicator Category: </b>{detail?.data?.category?.name}</li>
       <li><b>Data Type: </b>{detail?.data?.datatype}</li>
       <li><b>Frequency / Collection Period: </b>{detail?.data?.frequency}</li>
       <li><b>Data Source Type List: </b>
        <ul>
          {
          detail?.data?.data_source_types.map(item => {
