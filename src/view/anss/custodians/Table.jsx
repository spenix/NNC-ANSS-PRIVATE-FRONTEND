...
import { Row, Col, Button, Input, Table, Card, Tag, Space, Popconfirm, notification, Avatar, Dropdown, Menu, Modal } from "antd";
...
import { RiAddBoxLine, RiErrorWarningLine, RiEyeLine, RiImage2Line, RiDraftLine } from "react-icons/ri";
...
export default function TableList(props) {
  ...
  const confirmDel = (id) => {
    if(pageRoles.includes(ACTION_DELETE)){
      ...
      .then(()=> {
        openNotificationWithIcon("success", "Success", `International Custodian ${DELETED_DATA}`)
        fetch(pagination);
      }).catch((e) => {
        errMsg();
      });
  ...
  const btnForAddData = () => {
    if(pageRoles.includes(ACTION_ADD)){
      return (
        ...
        <Link to={`/international-custodians/forms/add-data/${window.btoa(0)}`}>
                  <Button
                    block
  ...
  //     return (
  //       <Link
  //                  to={`/international-custodians/forms/edit-data/${window.btoa(dataIndex)}`}
  //             >
  ...
  //       <Popconfirm
  //         placement="topLeft"
  //         title="Are you sure to delete this custodian?"
  ...
  //     return (
  //       <Link
  //                  to={`/international-custodians/forms/view-data/${window.btoa(dataIndex)}`}
  //             >
  ...
  const handleButtonClick = (e, record) => {
    ...
    switch (data[0]) {
      case "View":
        // history.push(`/international-custodians/forms/view-data/${window.btoa(record.key)}`);
        viewDataDetails(record);
        break;
      case "Edit":
        history.push(`/international-custodians/forms/edit-data/${window.btoa(record.key)}`)
        break;
      case "Delete":
        showConfirm(record, "Are you sure you want to delete this international custodian?");
        break;
      default:
        break;
    }
  ...
  const viewDataDetails = (detail) => {
    Modal.info({
      ...
      title: (
        <h5 className="hp-mb-0 hp-font-weight-500">View Custodian</h5>
      ),
      width:800,
      content: (
        <ul style={{ listStyle:"none" }}>
        <li><b>Logo: </b> {detail?.thumbnail ? (<Button size="small"  onClick={() => window.open(detail?.thumbnail, '_blank')}>View Logo</Button>) : "N/A"}</li>
